/* Test 5 - measures parallel insert and find (of random elements).
...
int main(int argc, char** argv) {
    ...
    std::vector<std::pair<uint32_t, uint32_t>> data_delete(load);
    std::generate(data_delete.begin(), data_delete.end(), [] { return std::make_pair(std::rand(), 1); });
    std::vector<std::pair<uint32_t, uint32_t>>* threads_data_delete[n_threads];
    for (int i = 0; i < n_threads; ++i) {
        threads_data_delete[i] = new std::vector<std::pair<uint32_t, uint32_t>>(data_delete.begin() + i*thread_load, data_delete.begin() + (i+1)*thread_load);
    }
    ...
    for (int i = 0; i < n_threads; ++i) {
        t[i] = std::thread(foo, &dynamic_pgm, threads_find_data[i], threads_data_delete[i], threads_insert_data[i], hasher(t->get_id()));
    }
