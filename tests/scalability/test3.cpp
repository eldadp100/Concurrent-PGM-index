/* Test 3 - measures delete of inserted elements.
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n_threads; ++i) {
        threads_data[i] = new std::vector<std::pair<uint32_t, uint32_t>>(data.begin() + i*thread_load, data.begin() + (i+1)*thread_load);
    }
